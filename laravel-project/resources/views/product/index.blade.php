...
<?php 
$images=DB::table('product_images')->where('product_id',$pro->id)->first();
?>
<td><img src="{{URL::to($images->name)}}" alt="" height="70px" width="80px"></td>
<td>{{$pro->product_name}}</td>
<td>{{$pro->product_code}}</td>
<td>{{str_limit($pro->details,$limit=50)}}</td>
<td>{{$pro->created_at}}</td>
<td>
   

<a href="{{URL::to('edit/product/'.$pro->id)}}"><button type="button"  class="btn">
      <i class="fa fa-edit" style="font-size:20px;color:#cccc00" id="edit" ></i></button></a>  
<form action="{{URL::to('delete/product/'.$pro->id)}}" method="GET" class="float-left" id="delete" >
          @csrf
     <button type="submit" class="btn btn-warning"> <i class="fa fa-trash" style="font-size:20px;color:#cc0000"></i></button>
      </form>
      <a href="{{route('product.show',$pro->id)}}"><button type="button" class="btn ">
...
<script>
	...
	if(!$this.hasClass('panel-collapsed')) {
        ...
        <ul class="cat">
                ...
                <li><a href="{{route('create.product')}}"> Добави нов продукт</a></li>
...
<div class="form-group row">
  ...
  <div class="col-md-4">
      <select name="brands" class="form-control "  >
        ...
        <option value="{{$brand ->id}}" name="role" @if (($bra)==$brand->id) selected @endif>{!! $brand ->name !!}</option>
    @endforeach
    </select>
    </div>
    </div>
    <div class="form-group row">
  <label for="brand" class="col-md-4 col-form-label text-md-right" >Категории </label>
  <div class="col-md-4">
      <select name="categories" class="form-control "  >
            <option value="Всички" name="category" ></option>
...
<div class="col-md-6" id="sort">
			<div class="panel panel-primary" >
				<div class="panel-heading">
					<h3 class="panel-title" style="  font-weight:bold">Подреди</h3>
					<span class="pull-right clickable"><i class="glyphicon glyphicon-chevron-down"></i></span>
        </div>
        <hr>
				<div class="panel-body">
       ...
       <li><a href="#"> @sortablelink('product_name','име','asc_suffix'   )</a></li>
       <li><a href="#">@sortablelink('product_price','цена ','asc_suffix'   )</a></li>
