...
<div class="row justify-content-center" id="create">
        <div class="col-md-8">
            <div class="card">
                <div class="card-header">Създадени категории</div>
  ...
  <thead>
    <tr>
    <th width="150px"> #</th>
    <th width="100px">Снимка</th>
<th width="150px"> Име</th>
<th width="200px">Детайли</th>
<th style="width:100px; min-width:100px;" class="text-center text-danger"><i class="fa fa-bolt"> </i><i class="fa fa-bolt"> </i></th>
    </tr>
  </thead>
  <tbody>
  @foreach($category as $cat)    <tr>
    <th scope="row">{{$cat->id}}</th>
<td><img src="{{URL::to($cat->logo)}}" alt="" height="70px" width="80px"></td>
<td>{{$cat->name}}</td>
<td>{{str_limit($cat->detail_about,$limit=50)}}</td>
<td>{{str_limit($cat->detail_contact,$limit=50)}}</td>
<td>{{str_limit($cat->detail_delivery,$limit=50)}}</td>

      <td id="action">
      <a href="{{ route('category.edit',$cat->id)}}"><button type="button"  class="btn">
      <i class="fa fa-edit" style="font-size:20px;color:#cccc00" id="edit" ></i></button></a>


      <form action="{{ route('category.destroy',$cat->id)}}" method="Get" class="float-left" id="delete" >

          @csrf
     <button type="submit" class="btn btn-warning" > <i class="fa fa-trash" style="font-size:20px;color:#cc0000"></i></button>
      </form>
      <a href="{{ route('category.show',$cat->id)}}"><button type="button" class="btn ">
...
<script>
	...
	if(!$this.hasClass('panel-collapsed')) {
        ...
        <ul class="cat">
                ...
                <li><a href="#"> Добави нова категория  </a></li>
...
<div class="form-group row">
  ...
  <div class="form-group row">
    ...
    <div class="col-md-6">
    <input type="text" class="form-control" id="filter" name="datails"  value="{{$details ?? ''}}">
    </div>
  </div>
  <div class="form-group row">
