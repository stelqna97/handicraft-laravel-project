...
@section('content')
    ...
    <div class="row justify-content-center" id="create">
        <div class="col-md-8">
            <div class="card">
                <div class="card-header">Регистрирани потребители</div>
  ...
  <thead>
    <tr>
      <th scope="col">#</th>
      <th scope="col">Профилна снимка</th>
      <th scope="col">Име</th>
  ...
  @foreach($users as $user)
    ...
    <td>                             <img src="{{URL::to($user->image)}}" alt="" height="70px" width="80px">
</td>
      <td> {{$user->name}}</td>
      <td>{{$user->email}}</td>
      ...
      <td id="action">
      <a href="{{route('admin.users.edit' , $user->id)}}"><button type="button"  class="btn">
      <i class="fa fa-edit" style="font-size:20px;color:#cccc00" id="edit" ></i></button></a>
        

      @if(Auth::user()->id!="$user->id")
      
      <form action="{{route('admin.users.destroy',$user->id)}}" method="GET" class="float-left" id="delete" >

          @csrf
     <button type="submit" class="btn btn-warning"> <i class="fa fa-trash" style="font-size:20px;color:#cc0000"></i></button>
      </form>
      @endif
      <a href="{{route('user.profile.show',$user->id)}}"><button type="button" class="btn ">
...
<form  method="GET">
  ...
  <div class="form-group row">
    <label for="filter" class="col-md-4 col-form-label text-md-right">Електронна поща</label>
    <div class="col-md-6">
    <input type="text" class="form-control" id="filter" name="email" value="{{$email ?? ''}}">
    </div>
  </div>
  <div class="form-group row">
  <label for="brand" class="col-md-4 col-form-label text-md-right" >Роля </label>
  <div class="col-md-4">
      <select name="roles" class="form-control "  >
            <option value="Всички" name="role" ></option>
    @foreach($roles as $role )
        <option value="{{$role ->id}}" name="role" @if (($roless)==$role->id) selected @endif>{!! $role ->name !!}</option>
    @endforeach
    </select>
    </div>
    </div>
  ...
  <div class="form-group row">
    <label for="filter" class="col-md-4 col-form-label text-md-right">Град</label>
    <div class="col-md-6">
    <input type="text" class="form-control" id="filter" name="city" value="{{$city ?? ''}}">
    </div>
  </div>
  ...
  <div class="form-group row">
<label for="role" class="col-md-4 col-form-label text-md-right">Пол </label>
      <div class="col-md-6" id="genre">
        <div class="form-check">
          <input class="form-check-input" type="checkbox"  name="genre" value="male"    {{ $genre == 'male' ? 'checked' : '' }}  id="gridRadios1"  >
        ...
        <div class="form-check">
          <input class="form-check-input"  type="checkbox" name="genre" value="female"    {{ $genre == 'female' ? 'checked' : '' }}  id="gridRadios2" >
...
<div class="col-md-6" id="sort">
       ...
       <li><a href="#"> @sortablelink('email','емайл','asc_suffix'   )</a></li>
       <li><a href="#">@sortablelink('name','име ','asc_suffix'   )</a></li>
       <li><a href="#"> @sortablelink('created_at','дата на създаване възходящ','asc_suffix'   )</a></li>
</ul>
  </div>
</div>
</div>
